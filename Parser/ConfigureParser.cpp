...
Parser* ConfigParseToConsole::Build(const std::string& filename)
{  try  {    pToker = new Toker;
    pToker->returnComments(false);
    pSemi = new SemiExp(pToker);
    pParser = new Parser(pSemi);
    pRepo = new Repository(pToker);   
	pRepo->_filename = filename;
	...
	if (!discardDisplay)
		pFunctionDefinition->addAction(pPrintFunction);	//add container rule and correpsonding action to parser while building it
	pContainer = new Container;
	pPrintContainer = new PrintContainer(pRepo);
	if (!discardDisplay)
	pContainer->addAction(pPrintContainer);
