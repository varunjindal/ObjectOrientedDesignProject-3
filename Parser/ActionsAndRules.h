...
public:
	...
	void doAction(ITokCollection*& ptrTc) {
		...
		elem->startLineCount = p_Repos->lineCount();
		p_Repos->tt->setValue(elem->name, elem->type, p_Repos->_filename, p_Repos->scopeStack().top()->name);
		p_Repos->scopeStack().top()->child.push_back(elem);
		p_Repos->scopeStack().push(elem);
		//p_Repos->tt->setValue(elem->name, elem->type, p_Repos->_filename, p_Repos->scopeStack().top()->name);
	}
};
...
public:
  ...
  void doAction(ITokCollection*& pTc)
  {
	...
	{
		//std::string name = (*pTc)[pTc->find(";") - 1];
		if (elem->name == "<" || elem->name == ">" || elem->name == "=" || elem->name == "==" || elem->name == "main") {
			//std::cout << "XYZ" << std::endl;
			//p_Repos->tt->setValue("X"+elem->name, "Member Function", p_Repos->_filename, p_Repos->scopeStack().top()->name);
			//std::cout << "Not a Global Function" << std::endl;
		}
		else
			//std::cout << "XYZ" << std::endl;
			p_Repos->tt->setValue(elem->name, "Member Function", p_Repos->_filename, p_Repos->scopeStack().top()->name);
			//std::cout << "Not a Global Function" << std::endl;
		
	}

	if (pTc->find("::") == pTc->length() && elem->name != "main")
	{
		//std::string name = (*pTc)[pTc->find(";") - 1];
		p_Repos->tt->setValue(elem->name, "Global Function", p_Repos->_filename, p_Repos->scopeStack().top()->name);
...
public:
	...
	void doAction(ITokCollection*& pTc)
	{
		...
		elem2->child.push_back(elem);
		p_Repos->scopeStack().push(elem);
		p_Repos->tt->setValue(elem->name, "Lambda", p_Repos->_filename, p_Repos->scopeStack().top()->name);
	}
};
...
public:
  ...
  void doAction(ITokCollection*& pTc)
  {
   ...
   // std::cout << "\n  Declaration: " << se.show();
	...
	{
		std::string name = (*pTc)[pTc->find(";") - 1];		
		p_Repos->tt->setValue(name, "typedef", p_Repos->_filename, p_Repos->scopeStack().top()->name);
	}
	...
	if (se.find("using") == 1)
	{
		...
		if (index > 0 && index < pTc->length()-1)
		{
			...
			p_Repos->tt->setValue(name, "using", p_Repos->_filename, "GNS");
		}
		
		/*std::cout << "\n	using :: " << name << std::endl;
		std::cout << "\n	namespace :: " << p_Repos->scopeStack().top()->name << std::endl;
	*/
		//std::cout << "\n " << "*******From Decl**************" << " \n";
...
public:                                   // initializers.  So eliminate
  ...
  {                                       // If you have two things left
    ...
    const static std::string keys[numKeys] = {
      ...
      "typedef", "typename", "unsigned", "volatile", "&", "*", "std", "::",
      "public", "protected", "private", ":", "typename", "++", "--","using"
    };
    for (int i = 0; i<numKeys; ++i)
    if (tok == keys[i])
      return true;
